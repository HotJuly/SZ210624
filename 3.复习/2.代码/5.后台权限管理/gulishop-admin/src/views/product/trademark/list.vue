<template>
  <div>
    <el-button 
    v-has-permission="`btn.Trademark.add`"
    ...
    <el-table :data="trademarkList" border style="width: 100%; margin: 20px 0">
      ...
      <el-table-column label="品牌LOGO">
        <template slot-scope="{ row }">
          <img style="width: 100px; height: 80px" :src="row.logoUrl"></img>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="{row}">
          ...
          <el-button 
            v-has-permission="`btn.Trademark.remove`" type="danger" size="mini" icon="el-icon-delete" @click="deleteAttr(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 
    ...
    <el-dialog :title="tmForm.id?'修改品牌':'添加品牌'" style="width:80%" :visible.sync="dialogFormVisible">
      <el-form ref="addForm" :model="tmForm" :rules="rules">
