...
- 3XX: 需要重定向, 浏览器直接跳转, 如302(将浏览器的地址重定向到新的地址),304(使用缓存,将资源获取地址引导到内存地址)
- 4XX: 客户端请求错误, 如:  403(代表具有这个资源,但是你没权限), 404(代表没有该资源,大概率是前端请求错误)
- 5XX: 服务器端错误, 如: 500
...
​	ajax(async javascript and xml)他其实并不是一种请求,他只是一种发送http请求的技术

- ajax请求是一种特别的http请求
- 对服务器端来说, 没有任何区别, 区别在浏览器端
  - 区别:HTTP请求不会报跨域错误,ajax会
...
- 同源: 协议, 域名, 端口, 三者都相同
  - 地址:http://www.baidu.com:8080/a?name=xiaoming
  - 地址=协议名+域名+端口号+查询字符串(请求路径+请求参数)
...
- 本身用于浏览器和Server通讯
- 被 "借用" 到本地存储
- 可用document.cookie读取或保存(但是服务器可以控制cookie是否能被修改和读取,cookie属性HttpOnly)
- 可以利用cookies工具库简化编码
...
- 实现两个标签页之间的通信
  - 方法一:通过storage进行通信
    - 一个页面绑定storage事件(注意:storage事件只能监视来自其他页面的storage修改,当前页面自身修改storage无法触发)
    - 另一个页面修改storage数据即可
  - 方法二:通过document.domain进行通信
    - 给当前页面设置document.domain为顶级域名
    - 使用window.open方法打开一个新页面
    - 在新页面也设置document.domain为顶级域名
    - 在新页面可以在全局添加任意数据
    - 旧页面通过window.open的返回值可以获取到新页面的window对象,从而获取数据
