...
<script>
	...
	export default {
		...
		async mounted(){
			/*
				onLoad中可以通过options形参接收到上个路由传递过来的query参数
				mounted中需要使用this.$mp接收上个路由传递过来的query参数
					注意:由于当前项目并没有使用VueRouter,所以不存在this.$route对象
			*/
		   // console.log('goodId',this.$mp.query.goodId)
		   
		   const result = await this.$axios('/getGoodDetail',{goodId:this.$mp.query.goodId})
		   if(result){
